...
object ThemeRepository {
    ...
    private val animatedThemes = listOf(
    ...
    private val solidThemes = listOf(
    ...
    fun selectableThemes(settings: KeyboardSettings): List<KeyboardTheme> {
        val custom = KeyboardTheme(
            "custom_color", "Custom", ThemeType.CUSTOM_COLOR,
            settings.customThemeColor, Color.parseColor("#0C0C10"),
            Color.WHITE, Color.parseColor("#040408")
        )
        val image = KeyboardTheme(
            "custom_image", "Photo", ThemeType.CUSTOM_IMAGE,
            Color.parseColor("#8C9EFF"), Color.parseColor("#CC16161C"),
            Color.WHITE, Color.BLACK
        )
        return listOf(defaultTheme) + animatedThemes + solidThemes + listOf(custom, image)
    }

    fun resolve(settings: KeyboardSettings): KeyboardTheme {
        return selectableThemes(settings).firstOrNull { it.id == settings.selectedThemeId }
            ?: defaultTheme
    }
